...
laser_ranges = []
laser_angles = []
bp=[[0.5,0.100],[1.5,0.27],[0.5,-0.27],[1.5,-0.100]]
...
def clbk_odom(msg):
    global position_
    global yaw_,active_,n,right_x_lower_limit,left_x_higher_limit,n
...
def go_to_goal(des_pos):
    ...
    if active_:
        ...
        pub.publish(velocity_message)
        # print ('x=', position_.x, ', y=',position_.y, ', distance to goal: ', distance)
...
def main():
    ...
    while not rospy.is_shutdown():
        if not active_:
            ...
            if state_ == 0:
                if position_.x>=right_x_lower_limit-0.5:
                    change_state(1)
                elif n==None and position_.x<left_x_higher_limit+0.4:
                    change_state(1)
                else:
                    go_to_goal(goal)
                # print('state 0')
